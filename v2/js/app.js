...
function load() {
  try {
    const raw = localStorage.getItem('onbd-proto-v2');
    if (raw) return Object.assign(DEFAULT_STATE(), JSON.parse(raw));
  } catch (e) {}
  return seedDetails(DEFAULT_STATE());
}
function save() { localStorage.setItem('onbd-proto-v2', JSON.stringify(S)); }
    ...
    <div class="landing-grid">
      <div>
        ...
        <div class="tcards">
          ...
          <div class="tcard optional" data-task="#/network" data-assume="A-43">
            ...
            <div class="t-main">
              ...
              <div class="t-why">${HIRE.manager} has named the people you’ll actually work with, and why each one matters</div>
              <div class="t-meta">
                <span class="m">${ic('clock.svg','sm')}No due date • Optional</span>
                ${booked ? `<span class="mini-prog"><span class="bar"><i style="width:${(booked/NETWORK.length)*100}%"></i></span>${booked} of ${NETWORK.length} booked</span>` : ''}
              </div>
            </div>
            <div class="t-side">
...
function landingRail() {
  return `
  <div class="rail">
    <div class="rail-card" data-assume="A-31 A-32">
      <h3>Your people</h3>
      ${contactRow(PEOPLE.manager, `<div class="c-extra">${am('A-32')}</div>`)}
      ${contactRow(PEOPLE.buddy, `<div class="c-extra">${ic('clock.svg','sm')} ${PEOPLE.buddy.tz}<br>${PEOPLE.buddy.assignedBy} ${am('A-31')}</div>`)}
...
function equipmentRows() {
  ...
  return [
    { item:'Computer', icon:'laptop.svg', by:`${HIRE.manager} — your hiring manager`, byCls:'mgr',
      ...
      unblock:`Their task: <b>“Order equipment for new hire”</b>. Nothing you can do from here.`, marker:'A-41' },
    { item:'Computer accessories', icon:'desktop.svg', by:'You', byCls:'you',
      status:accStatus, ok:E.submitted,
      unblock: E.submitted
        ? `Ordered from this task. Reference <b>INC6369627</b>.`
        : `This task — <b>“Order your workspace tech accessories”</b>, below.` },
    { item:'Mobile phone', icon:'mobile.svg', by:'You — from Day 1', byCls:'you',
      status:'NOT ORDERED YET', ok:false,
      unblock:`Task <b>“Order a phone”</b> opens on Day 1, and only if your role needs one.`, marker:'A-40' },
  ];
}
...
function renderEquipment() {
  ...
  <div class="page">
    ...
    <div class="eq-status" data-assume="A-41 A-42">
      <div class="eqs-h">
        <h3>Equipment orders</h3>
        <span class="eqs-note">Three items, three different owners. This table is the answer to “where is my equipment?” ${am('A-41')}</span>
      </div>
...
function equipmentSubmitted() {
  ...
  <div class="page">
    ...
    <div class="eq-status">
      <div class="eqs-h"><h3>Equipment orders</h3><span class="eqs-note">Still three items, three owners ${am('A-41')}</span></div>
      <table class="eq-table">
        <thead><tr><th>Item</th><th>Ordered by</th><th>Status</th><th>What unblocks it</th></tr></thead>
        <tbody>
          ${equipmentRows().map(r => `
...
function renderNetwork() {
  ...
  <div class="page">
    ...
    <div class="net-grid">
      <div>
        <div class="section-h"><h2>Five people, and why</h2><span class="hint">Optional — book what’s useful, ignore the rest</span></div>
        <div class="net-cards">
          ${NETWORK.map((p,i) => `
            <div class="net-card ${booked[i]?'booked':''}">
              <div class="nc-top">
                ...
                ${booked[i] ? `<span class="chip done">${ic('check.svg','sm')}${esc(booked[i])}</span>` : `<span class="chip waiting">Not booked</span>`}
              </div>
              <div class="nc-why">
              ...
              ${booked[i]
                ? `<div class="nc-slots"><button class="btn quiet sm" data-unbook="${i}">Cancel this 1:1</button></div>`
                : `<div class="nc-slots">
                    <span class="ncs-lbl">Suggested times:</span>
                    ${slotsFor(i).map(s => `<button class="slot-btn" data-book="${i}" data-slot="${esc(s)}">${s}</button>`).join('')}
                  </div>`}
            </div>`).join('')}
        </div>
      </div>
        ...
        <div class="rail-card">
          ...
          <div class="distinct">
            ...
            <div class="dist-row">
              ...
              <p>One person — ${PEOPLE.buddy.name} — for <b>culture and logistics</b>. Different purpose, different person.</p>
            </div>
            <div class="dist-row">
...
function renderAssumptions(highlightId) {
  ...
  $('#assumeBody').innerHTML = `
    ...
    marker like ${am('A-04')}. <b>${live} are marked on screen</b>, out of ${ASSUMPTIONS.length} in the register.
    Every entry carries where it came from — a third of them are now observed fact rather than inference.</div>
    ...
    <div class="a-entry retired">
      <div class="a-text"><b>OI-28</b> — equipment address dependency. Closed: shipping defaults to the office address, so
      the equipment task never depends on personal details.</div>
      <div class="a-resolve"><b>And:</b> the “false floor” concern behind OI-30’s framing is answered by one line of live copy —
      <i>“More to-dos may be assigned later.”</i> — now on the task list.</div>
    </div>

    <div class="ag-h">Open before build <span class="cnt">· ${OPEN_BEFORE_BUILD.length} questions, no marker</span></div>
...
function renderProtoDrawer() {
  $('#protoDrawer').innerHTML = `
    ...
    <div class="pc-links">
      ...
      <a data-goto="#/equipment">${ic('laptop.svg','sm')} Equipment — three owners, one status table</a>
      <a data-goto="#/network">${ic('users-connected.svg','sm')} Suggested network — proposed</a>
      <a data-openassume="1">${ic('list-tasks.svg','sm')} Assumptions &amp; gaps — ${ASSUMPTIONS.length} entries</a>
    </div>`;
}

function applyScenario(name) {
  const { persona, country, horizon } = S;
  S = seedDetails(DEFAULT_STATE());
  Object.assign(S, { persona, country, horizon, scenario: name });
  syncCountry();
...
document.addEventListener('click', e => {
  ...
  if (book) {
    ...
    toast(`1:1 booked — ${NETWORK[book.dataset.book].name}, ${book.dataset.slot}. They already know you’re starting.`, 'check-circle.svg');
    return;
  }
