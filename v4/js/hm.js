/* ============================================================
   Hiring Manager portal, v4 (AI-forward)
...
function hmTasks() {
  ...
  const list = [
    ...
    { id:'computer', label:'Equipment for your new hire', disp:'remove', route:'#/hm/computer',
      done: S.equipment.submitted, icon:'laptop.svg', marker:'A-60', dueOff:-7, sys:'ServiceNow', srcDue:true,
...
function hmBlockers() {
  ...
  if (S.hm.logistics.confirmed && S.pexUpdate) {
    out.push({ sev:'high', text:`People Experience moved orientation to finish at ${S.pex.blueprint.until}. The first-day details you confirmed no longer match, and Jordan is seeing the old version.`,
      action:'Re-confirm', route:'#/hm/logistics', marker:'M-32', short:'re-confirm the first-day details' });
  }
  if (!S.hm.buddy.assigned && days <= 21) {
    out.push({ sev:'high', text:'No buddy named yet. Nothing assigns one for you, so if you do not name somebody Jordan starts without a buddy.',
      action:'Name someone', route:'#/hm/buddy', marker:'M-07', short:'name a buddy' });
    ...
    out.push({ sev:'med', text:'The buddy has been asked but has not accepted. Nothing can go in their calendar until they do.',
      action:'See status', route:'#/hm/buddy', marker:'M-27', short:'wait for the buddy to accept' });
  }
  if (S.hm.card.needed === null && days <= 21) {
    out.push({ sev:'low', text:'The corporate card question is unanswered. If Jordan will travel, they need the agreement ready to sign on their second day.',
      action:'Answer it', route:'#/hm/card', short:'answer the corporate card question' });
  }
  if (!S.hm.software.confirmed) {
    out.push({ sev:'low', text:'We can’t suggest Jordan’s apps, because their role isn’t linked to a standard app list yet. That’s a gap in the system, not something you can fix here.',
      action:'See why', route:'#/hm/software', short:'pick Jordan’s apps by hand' });
  }
  return out;
}
...
function hmWaitingOnNewHire() {
  ...
  if (!S.startdate.confirmed && !S.startdate.changeRequested) out.push({ text:'The start date is not confirmed yet, so every other date is still provisional.' });
  if (S.startdate.changeRequested) out.push({ text:`Jordan has asked to start on ${esc(S.startdate.requestedDate)} instead. People Experience has the request.`, marker:'L-11' });
  return out;
}
...
/* New hire progress as the manager may see it (M-03 / L-05): names and
   status only, never content, sensitive items marked. v4 groups it the way
   the new hire's own journey is grouped (A-64): the three things before
   Day 1 are counted, the rest are shown with when they open. */
function nhProgressRows() {
  return [
    { key:'startdate', group:'pre', label:NH_TASK_LABELS.startdate, status:startdateStatus() },
    { key:'bgcheck', group:'pre', label:NH_TASK_LABELS.bgcheck, status:bgcheckStatus(), sensitive:true,
      hidden:'You can see the check is running. What it contains is never shown to you.' },
    { key:'equipment', group:'pre', label:NH_TASK_LABELS.equipment, status:equipmentStatus() },
    { key:'intro', group:'optional', label:NH_TASK_LABELS.intro, status:introStatus() },
    { key:'details', group:'day1', label:NH_TASK_LABELS.details, status:detailsStatus(), sensitive:true,
      hidden:'Everything in it stays in Workday. You see only that it is done.' },
    { key:'jd', group:'day1', label:NH_TASK_LABELS.jd, status:jdStatus() },
    { key:'policies', group:'week1', label:NH_TASK_LABELS.policies, status:policiesStatus(), sensitive:true,
      hidden:'Which notices were acknowledged, and when, is recorded for audit, not shown to you.' },
  ];
}
const NP_GROUPS = { pre:'Before Day 1', optional:'Optional', day1:'Day 1', week1:'First week' };
...
/* ============================================================
   ...
   ============================================================ */
/* Sidekick's read of the manager's position (A-73). Every clause comes from
   a rule above: blockers, overdue items and what Jordan still has open.
   Sidekick only puts them in a sentence; it does not decide what counts. */
function hmSummary() {
  const b = hmBlockers().filter(x => x.sev !== 'low'), lows = hmBlockers().filter(x => x.sev === 'low');
  const od = overdueItems(), w = hmWaitingOnNewHire();
  const parts = [];
  if (od.length) parts.push(`${od.length} thing${od.length > 1 ? 's are' : ' is'} overdue.`);
  if (b.length) parts.push(`This week, ${b.slice(0, 2).map(x => x.short).join(' and ')}.`);
  else if (lows.length) parts.push(`Nothing urgent. When you have a minute, ${lows[0].short}.`);
  else parts.push('Nothing needs you right now.');
  parts.push(w.length ? `${HIRE.preferred} has ${w.length} of their own still open, and nothing waits on you for those.`
                      : `${HIRE.preferred} is on track for ${fmtDate(startDate())}.`);
  return parts.join(' ');
}
...
function renderHmHome() {
  ...
  return `
  <section class="scene hm-scene" data-assume="M-02 A-73">
    <div class="scene-in">
      <div class="hm-hires" data-assume="M-13">
        ${HIRES.map((h,i) => `
          <button class="hire-tab ${i===0?'on':''} ${h.interactive?'':'flat'}" data-hire="${h.id}">
            ...
            <div class="ht-days">${h.interactive ? days : h.startsIn} days</div>
          </button>`).join('')}
    ...
    <div class="hm-grid">
      <div>
        ...
        <div class="nh-progress">
          ${Object.entries(NP_GROUPS).map(([g, label]) => `
            <div class="np-group">${label}</div>
            ${nhProgressRows().filter(r => r.group === g).map(r => `
            <div class="np-row">
              ...
              ${(g === 'day1' || g === 'week1') && r.status !== 'done'
                ? `<span class="chip waiting">${g === 'day1' ? 'Opens on Day 1' : 'First week'}</span>`
                : g === 'optional' && r.status === 'notstarted' ? `<span class="chip waiting">Optional</span>` : chip(r.status)}
            </div>`).join('')}`).join('')}
          <div class="np-note pnote">${ic('info-circle.svg','sm')}
            <span>You can see that Jordan has finished something, not what they put in it. Everything personal stays in
            Workday, and which notices they acknowledged is kept for audit, not shown to you.
...
function renderHmComputer() {
  ...
  <div class="page">
    ...
    <div class="task-head" data-assume="A-60">
      <h1>Equipment for your new hire ${am('A-60')}</h1>
  ...
  const slot = (role, person, title, hint, marker) => `
    <div class="form-sec meet-sec" ${marker ? `data-assume="${marker}"` : ''}>
      ...
      ${person ? `
        ...
        </div>` : `<div class="field">${picker(role, '')}</div>`}
    </div>`;
...
function renderHmWelcome() {
  ...
  // Read from the new hire's own journey (A-64), so the note can never
  // promise a task the portal no longer asks for.
  // Worded as things to do rather than screen titles, so each line reads as
  // a sentence. Anything without a phrase here falls back to its title.
  const say = t => WELCOME_TODO[t.key] || t.name.charAt(0).toLowerCase() + t.name.slice(1);
  const join = (a, last = ' and ') => a.length > 1 ? a.slice(0, -1).join(', ') + last + a[a.length - 1] : a[0];
  const firstWeek = [
...
function renderHmIntro() {
  ...
  // The same test as the task list: saved, consented and not empty. A draft
  // Jordan has not saved never shows here. The introduction is optional in
  // v4 (A-68), so the empty state is a normal one, not a missed deadline.
  const written = !!(S.intro.saved && S.intro.text.trim());
  const ready = written && S.intro.consent;
  return `
  <div class="page">
    ${hmCrumbs('Forward Jordan’s introduction')}
    <div class="task-head" data-assume="L-02">
      <h1>${ready ? 'Jordan wrote their introduction' : 'Jordan’s introduction'} ${am('L-02')}</h1>
      <p class="why">${ready
        ? 'They agreed you can share it with the team. It goes nowhere until you send it.'
        : 'It’s optional for Jordan, so it may not come. If they write one and agree to share it, it lands here, and it goes nowhere until you send it.'}</p>
      ${ready ? dispBanner('keep', 'This arrived because Jordan finished their side. You choose when the team sees it. Nothing is posted automatically.') : ''}
    </div>

    <div class="two-sec">
      <div class="sec-card">
        <div class="sec-top"><h2>What Jordan wrote</h2>
          ${ready ? `<span class="chip done">${ic('check.svg','sm')}Agreed to share</span>`
            : `<span class="chip waiting">${written ? 'Not agreed to share' : 'Nothing written yet'}</span>`}</div>
        <div class="pv-msg">
          ${hasPhoto
          ...
          <div class="pv-body">
            ...
            <div class="pv-text">${written ? esc(S.intro.text) : '<span class="pv-empty">Nothing written yet.</span>'}</div>
          </div>
        </div>
        <div class="share-model">
          ${ic('info-circle.svg')}
          <span>${ready
            ? 'Jordan ticked the box that lets you share this. Without that tick it would still be saved, but you would not be able to forward it.'
            : written
            ? 'Jordan hasn’t ticked the box that lets you share this, so it stays with them and you can’t forward it.'
            : 'Nothing reaches you until Jordan writes it and ticks the box that lets you share it.'}</span>
        </div>
        <div class="mt16" style="display:flex; gap:12px; align-items:center;">
          ${F
            ? `<span class="verified">${ic('check.svg','sm')}Forwarded to the team</span>
               <button class="btn quiet sm" id="introUnsend">Undo</button>`
            : `<button class="btn primary" id="introForward" ${ready ? '' : 'disabled'}>Forward to the team</button>`}
        </div>
      </div>
...
function handoffRows() {
  ...
  return [
    ...
    { dir:'nh', from:'Task progress', to:'Readiness view', marker:'L-05',
      ...
      hmRoute:'#/hm/', nhRoute:'#/' },
    // The AI layer's one handoff to a human (A-72): the red button lands in
    // the coordinator's queue with the conversation, not in a new inbox.
    { dir:'nh', from:'Sidekick: talk to a person', to:'Help requests, People Experience', marker:'L-13',
      done: S.sidekick.handoffs.length > 0,
      state: S.sidekick.handoffs.length
        ? `${S.sidekick.handoffs.length} question${S.sidekick.handoffs.length > 1 ? 's' : ''} passed on, in Maya’s Help requests queue with the conversation`
        : 'Nothing passed on yet. When it is, it lands in Maya’s Help requests queue',
      hmRoute:'#/pex/', nhRoute:'#/' },
  ];
}
...
function renderHandoffs() {
  ...
  <div class="page flow-page">
    ...
    <div class="ho-table">
      <div class="ho-head">
        <span>Manager or People Experience</span><span></span><span>New hire</span><span>State right now</span>
      </div>
      ${rows.map(r => `
