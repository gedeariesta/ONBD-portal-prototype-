...
// Resolve against the v4 root so this runs from anywhere, not just v3/.
const V3 = path.resolve(__dirname, '..');
const at = p => path.join(V3, p);
// Every script in js/, so a new file cannot hide its declarations (or the
// uses that keep another file's declarations alive) from this check.
